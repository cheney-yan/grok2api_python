...
class AuthTokenManager:
    ...
    def get_next_token_for_model(self, model_id, is_return=False):
        ...
        if token_entry:
            ...
            # 检查token是否已经超过限制，如果是，直接移除并获取下一个
            if token_entry["RequestCount"] >= token_entry["MaxRequestCount"]:
                logger.info(f"Token已达到使用上限 ({token_entry['RequestCount']}/{token_entry['MaxRequestCount']})，移除并尝试下一个", "TokenManager")
                self.remove_token_from_model(normalized_model, token_entry["token"])
                # 递归调用获取下一个可用token
                return self.get_next_token_for_model(model_id, is_return=False)
